...
return function(app_name)
  ...
  if icon_map_table[app_name] then
    return icon_map_table[app_name]
  end

  -- Check for pattern matches (Adobe apps with version numbers, etc.)
  if app_name:match("^Adobe Bridge") then return ":adobe_bridge:" end
  if app_name:match("^Adobe Illustrator") then return ":illustrator:" end
  if app_name:match("^Adobe InDesign") then return ":indesign:" end
  if app_name:match("^Adobe Photoshop") then return ":photoshop:" end
  if app_name:match("^MongoDB Compass") then return ":mongodb:" end
