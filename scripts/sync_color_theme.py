...
def set_yazi_theme(theme: str) -> str:
    ...
    if not path.exists():
        return f"yazi: skipped ({path} not found)"
    text = path.read_text(encoding="utf-8")
    text = re.sub(r'(?m)^dark\s*=.*$', f'dark  = "{theme}"', text)
    text = re.sub(r'(?m)^light\s*=.*$', f'light = "{theme}"', text)
    path.write_text(text, encoding="utf-8")
    return f"yazi: set dark/light -> {theme}"
...
def set_komorebi_theme(theme: str) -> list[str]:
    root = config_home() / "komorebi"
    return [
        _set_komorebi_theme_file(root / "komorebi.json", theme),
        _set_komorebi_theme_file(root / "komorebi.bar.json", theme),
    ]
