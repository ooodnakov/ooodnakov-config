...
- `Alt+Shift+X` / `Ctrl+Shift+X` / `Alt+Shift+H` / `Ctrl+Shift+A`: AI Commander command generation, last-results recall, prompt history, and split-pane ask mode. It reads API keys from `ANTHROPIC_API_KEY` and `OPENAI_API_KEY`, chooses Anthropic when both are present, and defaults the streaming renderer to `cat`; set `WEZTERM_AI_COMMANDER_RENDERER=sd` after installing `streamdown` if you want rich markdown rendering.
