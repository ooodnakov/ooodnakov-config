...
For optional tooling, `q` is installed via the system package manager when available, `uv` is installed via Astral's official installer, `bw` is installed from Bitwarden's pinned official native CLI archive, and `dua-cli` is installed from `https://github.com/byron/dua-cli` via `cargo`, avoiding distro-specific package naming drift.
