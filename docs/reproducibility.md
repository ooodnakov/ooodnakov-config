...
- GitHub release binaries already installed under the managed data tree are adopted into `bin` tracking via `scripts/security/bin_track.py` without re-downloading. Only binaries under `$HOME` are adopted, so system package-manager binaries (e.g. pacman-owned `bat` or `nvim`) stay with their package manager. This lets Topgrade's `bin update` step upgrade previously direct-downloaded tools such as `bw`, `rtk`, and `croc`.
