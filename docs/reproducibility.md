...
For Neovim, setup installs the pinned official GitHub release archive into the repo-managed data tree rather than relying on an OS package, so the managed `nvim` (validated `>= 0.10.0` for LazyVim) shadows any system binary on `PATH` and is adopted into `bin` tracking for Topgrade-driven updates.
...
- After dependency installation on Unix and PowerShell, `scripts/security/bin_track.py` scans catalog GitHub release tools in `~/.local/bin` and the managed data tree, including tools not selected in that run and managed binaries missing from PATH. `oooconf deps bin` therefore adopts earlier direct downloads such as `bw`, `rtk`, and `croc`. Adoption reads the executable's installed version (skipping unknown versions), preserves existing records, and previews changes under `--dry-run`. Resolved paths outside these install roots, including symlinks to system/package-manager binaries, are excluded. Invalid tracking JSON is reported and preserved.
- Topgrade runs in the foreground rather than under the setup spinner. On Unix, interactive runs attach directly to the controlling terminal so child tools retain terminal input/output; that output is not captured in the setup log. Noninteractive runs stream output through normal setup logging. Latest mode forwards `GITHUB_AUTH_TOKEN` (falling back to `GITHUB_TOKEN`), disables Topgrade self-update, retries failed steps twice, and reports remaining step failures as warnings unless `OOODNAKOV_DEPS_LATEST_STRICT=1` is set.
...
- Unix setup and doctor cover the configured custom Zsh plugins, including pinned `zsh-fzf-history-search` and `FFKL/s3cmd-zsh-plugin` (installed as `custom/plugins/s3cmd` to match its entrypoint). `zsh-autocomplete` is sourced before OMZ initializes completion so plugin `compdef` registrations survive warm-cache startup; syntax highlighting is the last OMZ plugin.
