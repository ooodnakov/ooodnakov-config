...
The root `justfile` exposes repeatable developer automation without replacing the underlying commands: `just check` runs Ruff linting, format verification, and pytest on every platform; `just unix` adds Bash syntax and smoke checks on Unix; `just completions` and `just lock` regenerate their tracked artifacts. Install the optional task runner with `oooconf deps just`.
