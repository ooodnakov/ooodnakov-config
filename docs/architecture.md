...
Autogenerated completion definitions are centralized in `scripts/autogen-completions.txt` and consumed by both setup implementations (`scripts/setup.sh` and `scripts/setup.ps1`) so completion generation stays aligned across shells.
