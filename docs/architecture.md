...
The project-owned `oooconf` command completions are generated separately from `scripts/oooconf-cli-spec.toml`, `scripts/oooconf-commands.txt`, and `scripts/optional-deps.toml` by `scripts/generate_oooconf_completions.py`.
