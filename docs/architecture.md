...
- `agents`: manage agent policy, MCP/RTK/skills workflows, and installation helpers
...
Autogenerated completion definitions are centralized in `scripts/autogen-completions.txt`, while `oooconf` command completion metadata is centralized in `scripts/oooconf-cli-spec.toml`; both are consumed by setup implementations (`scripts/setup.sh` and `scripts/setup.ps1`) so completion generation stays aligned across shells.
